...
def deposit_method_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Crypto Bot (USDT)", callback_data="deposit_crypto", icon_custom_emoji_id=EMOJI["crypto"]))
    builder.row(InlineKeyboardButton(text="Рубли (LZT.Market)", callback_data="deposit_rub", icon_custom_emoji_id=EMOJI["rub"]))
    builder.row(InlineKeyboardButton(text="Звёзды Telegram", callback_data="deposit_stars", icon_custom_emoji_id=EMOJI["star"]))
...
def privacy_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Политика конфиденциальности", url=PRIVACY_URL, icon_custom_emoji_id=EMOJI["info"]))
    builder.row(InlineKeyboardButton(text="Я ознакомился", callback_data="accept_privacy", icon_custom_emoji_id=EMOJI["check"], style="success"))
    return builder.as_markup()
...
def games_menu_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Блэкджек", callback_data="game_blackjack", icon_custom_emoji_id=EMOJI["blackjack"]))
    builder.row(InlineKeyboardButton(text="Слоты", callback_data="game_slots", icon_custom_emoji_id=EMOJI["slots"]))
    builder.row(InlineKeyboardButton(text="Рулетка", callback_data="game_roulette", icon_custom_emoji_id=EMOJI["roulette"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_menu_msg", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def blackjack_bet_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Своя сумма", callback_data="blackjack_bet_custom", icon_custom_emoji_id=EMOJI["edit"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def slots_bet_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Своя сумма", callback_data="slots_bet_custom", icon_custom_emoji_id=EMOJI["edit"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def roulette_bet_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Своя сумма", callback_data="roulette_bet_custom", icon_custom_emoji_id=EMOJI["edit"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def roulette_choice_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Число (x36)", callback_data="roulette_number", icon_custom_emoji_id=EMOJI["roulette"], style="primary"))
    builder.row(InlineKeyboardButton(text="Изменить ставку", callback_data="roulette_change_bet", icon_custom_emoji_id=EMOJI["edit"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def play_again_keyboard(game: str):
    ...
    builder.row(InlineKeyboardButton(text="Играть ещё", callback_data=cb, icon_custom_emoji_id=EMOJI.get(game, EMOJI["home"]), style="primary"))
...
async def create_lzt_invoice(amount_rub: float, user_id: int):
    """Создание счёта через LZT.Market - РАБОЧАЯ ВЕРСИЯ"""
    if not LZT_API_KEY:
...
async def cmd_admin(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        ...
        return
    await message.answer(f"{e(EMOJI['settings'])} <b>Админ-панель</b>", parse_mode=ParseMode.HTML, reply_markup=admin_panel_keyboard())
...
async def games_menu(message: Message):
    await message.answer(
        ...
        parse_mode=ParseMode.HTML, reply_markup=games_menu_keyboard()
    )
...
"""
    ...
    await message.answer(
        f"{e(EMOJI['wallet'])} <b>ПОПОЛНЕНИЕ БАЛАНСА</b>\n\n{e(EMOJI['crypto'])} Crypto Bot\n{e(EMOJI['rub'])} Рубли (LZT)\n{e(EMOJI['star'])} Звёзды",
        parse_mode=ParseMode.HTML, reply_markup=deposit_method_keyboard()
    )
...
async def accept_privacy_callback(callback: CallbackQuery):
    ...
    await callback.message.answer(f"{e(EMOJI['check'])} <b>Спасибо!</b>\n\nТеперь вы можете пополнить баланс.", parse_mode=ParseMode.HTML)
...
"""
            msg = await message.answer(text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
            asyncio.create_task(lzt_payment_check_loop(message.from_user.id, msg, amount_usdt, amount_rub, invoice_data["invoice_id"]))
        else:
            await message.answer(
                f"{e(EMOJI['cross'])} Ошибка создания платежа.\n\nПроверьте API ключ или попробуйте Crypto Bot.\n\nПоддержка: @{SUPPORT_USERNAME}",
                parse_mode=ParseMode.HTML
            )
    except ValueError:
        await message.answer(f"{e(EMOJI['cross'])} Введите число", parse_mode=ParseMode.HTML)
...
async def withdraw_amount(message: Message, state: FSMContext):
    try:
        ...
        if check_data:
            ...
            await message.answer(
                f"{e(EMOJI['check'])} <b>ЧЕК НА ВЫВОД!</b>\n\n{e(EMOJI['money'])} {amount:.2f} USDT\n{e(EMOJI['link'])} <b><a href='{check_link}'>ПОЛУЧИТЬ</a></b>\n\n{e(EMOJI['wallet'])} Баланс: <b>{new_balance:.2f} USDT</b>",
                parse_mode=ParseMode.HTML, disable_web_page_preview=True, reply_markup=main_menu_keyboard()
            )
        else:
...
async def help_cmd(message: Message):
    await message.answer(
        f"{e(EMOJI['info'])} <b>ПОМОЩЬ</b>\n\n{e(EMOJI['wallet'])} Пополнение: Crypto Bot / LZT / Звёзды\nКурс: 1 USDT = {USDT_RUB_RATE}₽\nВывод от 0.5 USDT\nСтавки от 0.1 USDT\n\n{e(EMOJI['settings'])} /admin /id /support",
        parse_mode=ParseMode.HTML
    )
...
async def game_dice(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=dice_mode_keyboard()
    )
    await callback.answer()
...
async def dice_mode_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=dice_bet_keyboard()
    )
    await callback.answer()
...
async def dice_set_bet(callback: CallbackQuery, state: FSMContext):
    ...
    bet = float(bet_type)
    if user["balance"] < bet: await callback.answer("Недостаточно средств!", show_alert=True); return
    if bet < 0.1: await callback.answer("Мин. 0.1 USDT", show_alert=True); return
    ...
    if mode in ["twodice", "threedice"]:
        ...
        await callback.message.edit_text(
            ...
            parse_mode=ParseMode.HTML, reply_markup=dice_choice_keyboard(mode)
        )
    await callback.answer()

async def dice_play_auto(callback, user_id, bet, mode):
    user = get_user(user_id)
    if mode == "twodice":
        d1, d2 = random.randint(1,6), random.randint(1,6)
        is_win = (d1+d2)==7; mult=4; result=f"Кубики: {d1}+{d2}={d1+d2}"
    else:
        d1,d2,d3 = random.randint(1,6),random.randint(1,6),random.randint(1,6)
        total=d1+d2+d3; is_win=10<=total<=11; mult=3; result=f"Кубики: {d1}+{d2}+{d3}={total}"
    
    if is_win:
        win_amount=bet*mult; update_balance(user_id, win_amount-bet); add_game_stat(user_id,"dice",True)
        text=f"{e(EMOJI['dice'])} <b>КУБИК</b>\n\n{result}\n\n{e(EMOJI['check'])} <b>ПОБЕДА!</b>\n\n{e(EMOJI['money'])} +{win_amount-bet:.2f} USDT"
    else:
        update_balance(user_id,-bet); add_game_stat(user_id,"dice",False)
...
async def dice_play(callback: CallbackQuery):
    user_id = callback.from_user.id
    if user_id not in user_bets: await callback.answer("Выберите ставку!", show_alert=True); return
    
    bet_data = user_bets[user_id]; bet = bet_data["bet"]; mode = bet_data["mode"]
    user = get_user(user_id)
    if user["balance"] < bet: await callback.answer("Недостаточно средств!", show_alert=True); return
    
    choice = callback.data; roll = random.randint(1,6); is_win = False
    ...
    if mode == "highlow":
        ...
        num = int(choice.split("_")[-1]); is_win = roll == num; choice_text = f"Число {num}"
    elif mode == "lucky":
        num = int(choice.split("_")[-1]); is_win = roll == num; choice_text = f"Число {num}"
...
async def game_basketball(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=basketball_bet_keyboard()
    )
    await callback.answer()

@router.callback_query(F.data.startswith("basketball_bet_"))
async def basketball_set_bet(callback: CallbackQuery):
    parts = callback.data.split("_"); bet_type = parts[2]
    user_id = callback.from_user.id; user = get_user(user_id)
    
    if bet_type == "custom":
        await callback.message.edit_text(f"{e(EMOJI['basketball'])} <b>БАСКЕТБОЛ</b>\n\n{e(EMOJI['wallet'])} Баланс: {user['balance']:.2f} USDT\n\nВведите сумму (мин. 0.1):", parse_mode=ParseMode.HTML)
        user_bets[user_id] = {"game": "basketball", "awaiting_custom": True}; await callback.answer(); return
    ...
    await callback.message.edit_text(
        f"{e(EMOJI['basketball'])} <b>БАСКЕТБОЛ</b>\n\n{e(EMOJI['money'])} Ставка: <b>{bet:.2f} USDT</b>\n{e(EMOJI['wallet'])} Баланс: <b>{user['balance']:.2f} USDT</b>\n\nВыберите исход:",
        parse_mode=ParseMode.HTML, reply_markup=basketball_choice_keyboard()
    )
    await callback.answer()
...
async def basketball_play(callback: CallbackQuery):
    ...
    bet = user_bets[user_id]["bet"]; user = get_user(user_id)
    if user["balance"] < bet: await callback.answer("Недостаточно средств!", show_alert=True); return
    
    choice = callback.data; is_win = random.random() < 0.2
    
    if choice == "basketball_hit": choice_text, mult = "Попадание", 1.85; result_desc = "Попадание!" if is_win else "Промах!"
    elif choice == "basketball_miss": choice_text, mult = "Промах", 1.5; result_desc = "Промах!" if is_win else "Попадание!"
    else: choice_text, mult = "2 раза", 3; is_win = random.random() < 0.04; result_desc = "2 раза!" if is_win else "Промах!"
    
    if is_win:
        win_amount=bet*mult; update_balance(user_id,win_amount-bet); add_game_stat(user_id,"basketball",True)
        text=f"{e(EMOJI['basketball'])} <b>БАСКЕТБОЛ</b>\n\nВыбор: <b>{choice_text}</b>\nРезультат: <b>{result_desc}</b>\n\n{e(EMOJI['check'])} <b>ПОБЕДА!</b>\n\n{e(EMOJI['money'])} +{win_amount-bet:.2f} USDT"
    else:
        update_balance(user_id,-bet); add_game_stat(user_id,"basketball",False)
        text=f"{e(EMOJI['basketball'])} <b>БАСКЕТБОЛ</b>\n\nВыбор: <b>{choice_text}</b>\nРезультат: <b>{result_desc}</b>\n\n{e(EMOJI['cross'])} <b>ПОРАЖЕНИЕ</b>\n\n{e(EMOJI['money'])} -{bet:.2f} USDT"
    text += f"\n{e(EMOJI['wallet'])} Баланс: <b>{get_user(user_id)['balance']:.2f} USDT</b>"
...
async def game_football(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=football_mode_keyboard()
    )
    await callback.answer()
...
async def football_mode_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=football_bet_keyboard()
    )
    await callback.answer()
...
async def football_set_bet(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_"); bet_type = parts[2]
    user_id = callback.from_user.id; user = get_user(user_id)
    data = await state.get_data(); mode = data.get("football_mode", "goal")
    
    if bet_type == "custom":
        await callback.message.edit_text(f"{e(EMOJI['football'])} <b>ФУТБОЛ</b>\n\n{e(EMOJI['wallet'])} Баланс: {user['balance']:.2f} USDT\n\nВведите сумму (мин. 0.1):", parse_mode=ParseMode.HTML)
        user_bets[user_id] = {"game": "football", "mode": mode, "awaiting_custom": True}; await callback.answer(); return
...
async def football_play_auto(callback, user_id, bet, mode):
    user = get_user(user_id)
    mults = {"goal": 1.85, "miss": 1.5, "penalty": 2, "post": 4}; mult = mults[mode]
    names = {"goal": "Гол", "miss": "Промах", "penalty": "Пенальти", "post": "Штанга/Перекладина"}
    
    if mode == "post": is_win = random.random() < 0.1; result = "Штанга!" if is_win else "Мимо!"
    elif mode == "penalty": is_win = random.random() < 0.3; result = "ГОЛ!" if is_win else "Вратарь!"
    else:
        is_win = random.random() < 0.2
        if mode == "goal": result = "ГОЛ!" if is_win else "Промах!"
        else: result = "Промах!" if is_win else "ГОЛ!"
    
    if is_win:
        win_amount=bet*mult; update_balance(user_id,win_amount-bet); add_game_stat(user_id,"football",True)
        text=f"{e(EMOJI['football'])} <b>ФУТБОЛ</b>\n\nРежим: <b>{names[mode]}</b>\nРезультат: <b>{result}</b>\n\n{e(EMOJI['check'])} <b>ПОБЕДА!</b>\n\n{e(EMOJI['money'])} +{win_amount-bet:.2f} USDT"
    else:
        update_balance(user_id,-bet); add_game_stat(user_id,"football",False)
        text=f"{e(EMOJI['football'])} <b>ФУТБОЛ</b>\n\nРежим: <b>{names[mode]}</b>\nРезультат: <b>{result}</b>\n\n{e(EMOJI['cross'])} <b>ПОРАЖЕНИЕ</b>\n\n{e(EMOJI['money'])} -{bet:.2f} USDT"
    text += f"\n{e(EMOJI['wallet'])} Баланс: <b>{get_user(user_id)['balance']:.2f} USDT</b>"
...
async def game_blackjack(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=blackjack_bet_keyboard()
    )
    await callback.answer()
...
async def blackjack_set_bet(callback: CallbackQuery):
    ...
    if bet_type == "custom":
        await callback.message.edit_text(f"{e(EMOJI['blackjack'])} <b>БЛЭКДЖЕК</b>\n\n{e(EMOJI['wallet'])} Баланс: {user['balance']:.2f} USDT\n\nВведите сумму (мин. 0.1):", parse_mode=ParseMode.HTML)
        user_bets[user_id] = {"game": "blackjack", "awaiting_custom": True}; await callback.answer(); return
    ...
    await callback.message.edit_text(
        f"{e(EMOJI['blackjack'])} <b>БЛЭКДЖЕК</b>\n\n{e(EMOJI['money'])} Ставка: <b>{bet:.2f} USDT</b>\n\nВаши карты: <b>{format_hand(player_hand)}</b> ({player_value})\nКарта дилера: <b>{dealer_hand[0]}</b> + ?\n\nВыберите действие:",
        parse_mode=ParseMode.HTML, reply_markup=blackjack_game_keyboard()
    )
    await callback.answer()
...
async def blackjack_play(callback: CallbackQuery):
    ...
    game_data = user_bets[user_id]; bet = game_data["bet"]
    deck = game_data["deck"]; player_hand = game_data["player_hand"]
    ...
    if callback.data == "bj_hit":
        ...
        if player_value > 21:
            update_balance(user_id, -bet); add_game_stat(user_id, "blackjack", False)
            dealer_value = get_hand_value(dealer_hand)
            text = f"{e(EMOJI['blackjack'])} <b>БЛЭКДЖЕК</b>\n\nВаши карты: <b>{format_hand(player_hand)}</b> ({player_value}) — <b>ПЕРЕБОР!</b>\nКарты дилера: <b>{format_hand(dealer_hand)}</b> ({dealer_value})\n\n{e(EMOJI['cross'])} <b>ПОРАЖЕНИЕ</b>\n\n{e(EMOJI['money'])} -{bet:.2f} USDT"
            text += f"\n{e(EMOJI['wallet'])} Баланс: <b>{get_user(user_id)['balance']:.2f} USDT</b>"
            await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=play_again_keyboard("blackjack"))
            del user_bets[user_id]; await callback.answer(); return
    ...
    # Stand
    while get_hand_value(dealer_hand) < 17: dealer_hand.append(deck.pop())
    player_value = get_hand_value(player_hand); dealer_value = get_hand_value(dealer_hand)
    
    if dealer_value > 21 or player_value > dealer_value:
        win_amount = bet * 2; update_balance(user_id, win_amount - bet); add_game_stat(user_id, "blackjack", True)
        text = f"{e(EMOJI['blackjack'])} <b>БЛЭКДЖЕК</b>\n\nВаши карты: <b>{format_hand(player_hand)}</b> ({player_value})\nКарты дилера: <b>{format_hand(dealer_hand)}</b> ({dealer_value})\n\n{e(EMOJI['check'])} <b>ПОБЕДА!</b>\n\n{e(EMOJI['money'])} +{win_amount - bet:.2f} USDT"
    ...
    text += f"\n{e(EMOJI['wallet'])} Баланс: <b>{get_user(user_id)['balance']:.2f} USDT</b>"
    await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=play_again_keyboard("blackjack"))
    del user_bets[user_id]; await callback.answer()
...
async def game_slots(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=slots_bet_keyboard()
    )
    await callback.answer()
...
async def slots_set_bet(callback: CallbackQuery):
    ...
    if bet_type == "custom":
        await callback.message.edit_text(f"{e(EMOJI['slots'])} <b>СЛОТЫ</b>\n\n{e(EMOJI['wallet'])} Баланс: {user['balance']:.2f} USDT\n\nВведите сумму (мин. 0.1):", parse_mode=ParseMode.HTML)
        user_bets[user_id] = {"game": "slots", "awaiting_custom": True}; await callback.answer(); return
    ...
    # Крутим слоты
    s1 = random.choice(SLOT_SYMBOLS); s2 = random.choice(SLOT_SYMBOLS); s3 = random.choice(SLOT_SYMBOLS)
    ...
    if mult > 0:
        win_amount = bet * mult; update_balance(user_id, win_amount - bet); add_game_stat(user_id, "slots", True)
        text = f"{e(EMOJI['slots'])} <b>СЛОТЫ</b>\n\n[{s1}] [{s2}] [{s3}]\n\n{e(EMOJI['check'])} <b>ВЫИГРЫШ x{mult}!</b>\n\n{e(EMOJI['money'])} +{win_amount - bet:.2f} USDT"
    else:
        update_balance(user_id, -bet); add_game_stat(user_id, "slots", False)
...
async def game_roulette(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode=ParseMode.HTML, reply_markup=roulette_bet_keyboard()
    )
    await callback.answer()
...
async def roulette_set_bet(callback: CallbackQuery):
    ...
    if bet_type == "custom":
        await callback.message.edit_text(f"{e(EMOJI['roulette'])} <b>РУЛЕТКА</b>\n\n{e(EMOJI['wallet'])} Баланс: {user['balance']:.2f} USDT\n\nВведите сумму (мин. 0.1):", parse_mode=ParseMode.HTML)
        user_bets[user_id] = {"game": "roulette", "awaiting_custom": True}; await callback.answer(); return
    ...
    await callback.message.edit_text(
        f"{e(EMOJI['roulette'])} <b>РУЛЕТКА</b>\n\n{e(EMOJI['money'])} Ставка: <b>{bet:.2f} USDT</b>\n{e(EMOJI['wallet'])} Баланс: <b>{user['balance']:.2f} USDT</b>\n\nВыберите исход:",
        parse_mode=ParseMode.HTML, reply_markup=roulette_choice_keyboard()
    )
    await callback.answer()
...
async def roulette_play(callback: CallbackQuery):
    ...
    if user_id not in user_bets or user_bets[user_id].get("game") != "roulette":
        await callback.answer("Выберите ставку!", show_alert=True); return
    ...
    if choice == "red": is_win, mult, choice_text = is_red, 2, "Красное"
    elif choice == "black": is_win, mult, choice_text = is_black, 2, "Чёрное"
    elif choice == "even": is_win, mult, choice_text = is_even, 2, "Чётное"
    elif choice == "odd": is_win, mult, choice_text = is_odd, 2, "Нечётное"
    elif choice == "number":
        await callback.message.edit_text(
            f"{e(EMOJI['roulette'])} <b>РУЛЕТКА</b>\n\n{e(EMOJI['money'])} Ставка: <b>{bet:.2f} USDT</b>\n\nВведите число от 0 до 36:",
            parse_mode=ParseMode.HTML
        )
        user_bets[user_id]["mode"] = "number"; await callback.answer(); return
    
    if is_win:
        win_amount = bet * mult; update_balance(user_id, win_amount - bet); add_game_stat(user_id, "roulette", True)
        text = f"{e(EMOJI['roulette'])} <b>РУЛЕТКА</b>\n\nВыбор: <b>{choice_text}</b>\nВыпало: {color} <b>{number}</b>\n\n{e(EMOJI['check'])} <b>ПОБЕДА!</b>\n\n{e(EMOJI['money'])} +{win_amount - bet:.2f} USDT"
    else:
        update_balance(user_id, -bet); add_game_stat(user_id, "roulette", False)
        text = f"{e(EMOJI['roulette'])} <b>РУЛЕТКА</b>\n\nВыбор: <b>{choice_text}</b>\nВыпало: {color} <b>{number}</b>\n\n{e(EMOJI['cross'])} <b>ПОРАЖЕНИЕ</b>\n\n{e(EMOJI['money'])} -{bet:.2f} USDT"
...
async def handle_custom_bet(message: Message, state: FSMContext):
    ...
    if user_id not in user_bets or not user_bets[user_id].get("awaiting_custom"):
        ...
        if user_id in user_bets and user_bets[user_id].get("game") == "roulette" and user_bets[user_id].get("mode") == "number":
            try:
                ...
                if chosen_num < 0 or chosen_num > 36: raise ValueError
    ...
    try:
        ...
        if bet < 0.1:
            await message.answer(f"{e(EMOJI['cross'])} Минимум 0.1 USDT", parse_mode=ParseMode.HTML); return
        if user["balance"] < bet:
            await message.answer(f"{e(EMOJI['cross'])} Недостаточно средств!", parse_mode=ParseMode.HTML); return
...
async def admin_panel(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS: await callback.answer("Нет доступа", show_alert=True); return
    await callback.message.edit_text(f"{e(EMOJI['settings'])} <b>Админ-панель</b>", parse_mode=ParseMode.HTML, reply_markup=admin_panel_keyboard())
    await callback.answer()
...
async def admin_users_list(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS: await callback.answer("Нет доступа", show_alert=True); return
    db = get_all_users()
    text = f"{e(EMOJI['users'])} <b>ПОЛЬЗОВАТЕЛИ:</b>\n\n" if db else f"{e(EMOJI['users'])} <b>Пользователей нет</b>"
...
async def admin_edit_balance_process(message: Message, state: FSMContext):
    ...
    try:
        ...
        user_id = int(parts[0]); new_balance = float(parts[1].replace(",","."))
        if new_balance < 0: raise ValueError
        
        db = get_all_users(); uid = str(user_id)
        if uid not in db:
            await message.answer(f"{e(EMOJI['cross'])} Пользователь не найден", parse_mode=ParseMode.HTML, reply_markup=cancel_keyboard()); return
        
        old_balance, new_balance = set_balance(user_id, new_balance)
        name = db[uid].get("first_name","") or db[uid].get("username","") or user_id
        
        try: await bot.send_message(user_id, f"{e(EMOJI['wallet'])} Баланс изменён!\n\n{e(EMOJI['edit'])} Новый: <b>{new_balance:.2f} USDT</b>", parse_mode=ParseMode.HTML)
        except: pass
...
async def admin_broadcast_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS: await callback.answer("Нет доступа", show_alert=True); return
    await callback.message.edit_text(f"{e(EMOJI['broadcast'])} <b>РАССЫЛКА</b>\n\n{e(EMOJI['info'])} Отправьте сообщение:", parse_mode=ParseMode.HTML, reply_markup=cancel_keyboard())
...
async def admin_broadcast_send(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS: await state.clear(); return
    db = get_all_users(); success = failed = 0
