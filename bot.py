...
if not YOOMONEY_CLIENT_ID:
    raise ValueError("Переменная окружения YOOMONEY_CLIENT_ID не установлена!")
...
USDT_RUB_RATE = 90  # 1 USDT = 90 RUB
...
# ========== YOOMONEY API ==========
async def create_yoomoney_payment(amount_rub: float, user_id: int):
    """Создание платежа через YooMoney"""
    async with aiohttp.ClientSession() as session:
        # Формируем ссылку для оплаты
        payment_url = f"https://yoomoney.ru/quickpay/confirm.xml"
        params = {
            "receiver": YOOMONEY_CLIENT_ID,
            "formcomment": f"Vest Casino - пополнение баланса для user_{user_id}",
            "short-dest": f"Vest Casino пополнение",
            "label": f"user_{user_id}",
            "quickpay-form": "shop",
            "targets": f"Пополнение баланса на {amount_rub / USDT_RUB_RATE:.2f} USDT",
            "sum": amount_rub,
            "paymentType": "AC",
        }
...
user_bets = {}
user_invoices = {}
pending_rub_payments = {}  # user_id: {"amount_usdt": float, "amount_rub": float}
...
💎 <b>Crypto Bot</b> — пополнение в USDT
₽ <b>Рубли</b> — через YooMoney (курс 1 USDT = {USDT_RUB_RATE}₽)
"""
...
"""
            builder = InlineKeyboardBuilder()
            builder.row(InlineKeyboardButton(text="Проверить оплату", callback_data=f"check_crypto_{invoice['invoice_id']}", icon_custom_emoji_id=EMOJI["loading"]))
            builder.row(InlineKeyboardButton(text="Отмена", callback_data="cancel_action", icon_custom_emoji_id=EMOJI["cross"]))
...
async def deposit_rub_amount(message: Message, state: FSMContext):
    try:
        ...
        payment_data = await create_yoomoney_payment(amount_rub, message.from_user.id)
        ...
        text = f"""
₽ <b>СЧЁТ НА ОПЛАТУ (YooMoney)</b>

{e(EMOJI['money'])} Сумма: <b>{amount_usdt:.2f} USDT</b> ({amount_rub}₽)
Курс: 1 USDT = {USDT_RUB_RATE}₽

{e(EMOJI['link'])} <b><a href='{payment_data['pay_url']}'>НАЖМИТЕ ДЛЯ ОПЛАТЫ</a></b>

После оплаты нажмите кнопку ниже:
"""
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"confirm_rub_{message.from_user.id}", icon_custom_emoji_id=EMOJI["check"]))
        builder.row(InlineKeyboardButton(text="Отмена", callback_data="cancel_action", icon_custom_emoji_id=EMOJI["cross"]))
...
@router.callback_query(F.data.startswith("confirm_rub_"))
async def confirm_rub_payment(callback: CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    
    if user_id != callback.from_user.id and callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Это не ваш платёж!", show_alert=True)
        return
    
    if user_id in pending_rub_payments:
        payment = pending_rub_payments[user_id]
        update_balance(user_id, payment["amount_usdt"])
        ...
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_message(
                    ...
                    f"💎 Сумма: {payment['amount_usdt']:.2f} USDT\n"
                    f"₽ Сумма: {payment['amount_rub']}₽\n\n"
                    f"<i>Проверьте поступление средств в YooMoney!</i>",
                    parse_mode=ParseMode.HTML
                )
            except:
                pass
        ...
        await callback.message.edit_text(
            f"{e(EMOJI['check'])} <b>ЗАЯВКА ПРИНЯТА!</b>\n\n"
            f"Средства будут зачислены после проверки администратором.\n\n"
            f"{e(EMOJI['info'])} Обычно проверка занимает до 15 минут.",
            parse_mode=ParseMode.HTML,
...
@router.callback_query(F.data.startswith("approve_rub_"))
async def approve_rub_payment(callback: CallbackQuery):
...
• Пополнение от 0.1 USDT (Crypto Bot или RUB)
