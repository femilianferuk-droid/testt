...
def load_media_db():
    try:
        with open(MEDIA_DB_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        return {
            "profile": {"type": None, "file_id": None},
            "games": {"type": None, "file_id": None},
            "deposit": {"type": None, "file_id": None},
            "withdraw": {"type": None, "file_id": None},
            "support": {"type": None, "file_id": None},
            "help": {"type": None, "file_id": None}
        }

def save_media_db(data):
    with open(MEDIA_DB_FILE, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=4, ensure_ascii=False)
    ...
    if uid not in db:
        db[uid] = {
            ...
            "stats": {
                ...
                "bowling": {"wins": 0, "losses": 0},
                "guess_number": {"wins": 0, "losses": 0},
                "total_won": 0,
                "total_lost": 0
            }
        }
...
EMOJI = {
    ...
    "blackjack": "5373141891321699086",
    "bowling": "5370810157871667232",
    "guess": "5471984997361523302",
    ...
    "up": "5963103826075456248",
    "down": "6039802767931871481",
}
...
class GameStates(StatesGroup):
    waiting_for_guess_number = State()
...
def deposit_method_keyboard():
    ...
    builder.row(InlineKeyboardButton(
        text="Рубли (Поддержка)",
        callback_data="deposit_rub_support",
...
def games_menu_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Боулинг", callback_data="game_bowling", icon_custom_emoji_id=EMOJI["bowling"]))
    builder.row(InlineKeyboardButton(text="Угадай число", callback_data="game_guess_number", icon_custom_emoji_id=EMOJI["guess"]))
...
def admin_panel_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="Медиа", callback_data="admin_media", icon_custom_emoji_id=EMOJI["media"]))
...
def admin_media_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Медиа в Профиль", callback_data="admin_media_profile", icon_custom_emoji_id=EMOJI["profile"]))
    builder.row(InlineKeyboardButton(text="Медиа в Игры", callback_data="admin_media_games", icon_custom_emoji_id=EMOJI["games"]))
    builder.row(InlineKeyboardButton(text="Медиа в Пополнение", callback_data="admin_media_deposit", icon_custom_emoji_id=EMOJI["wallet"]))
    builder.row(InlineKeyboardButton(text="Медиа в Вывод", callback_data="admin_media_withdraw", icon_custom_emoji_id=EMOJI["withdraw"]))
    builder.row(InlineKeyboardButton(text="Медиа в Поддержку", callback_data="admin_media_support", icon_custom_emoji_id=EMOJI["support"]))
    builder.row(InlineKeyboardButton(text="Медиа в Помощь", callback_data="admin_media_help", icon_custom_emoji_id=EMOJI["info"]))
    builder.row(InlineKeyboardButton(text="Удалить все медиа", callback_data="admin_media_clear", icon_custom_emoji_id=EMOJI["cross"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="admin_panel", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def blackjack_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Взять", callback_data="bj_hit", icon_custom_emoji_id=EMOJI["blackjack"]),
        InlineKeyboardButton(text="Пас", callback_data="bj_stand", icon_custom_emoji_id=EMOJI["cross"])
    )
    return builder.as_markup()
...
def basketball_choice_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Попадание (1.85x)", callback_data="basketball_hit", icon_custom_emoji_id=EMOJI["basketball"]))
    builder.row(InlineKeyboardButton(text="Промах (1.5x)", callback_data="basketball_miss", icon_custom_emoji_id=EMOJI["basketball"]))
    builder.row(InlineKeyboardButton(text="Попадание 2 раза (3x)", callback_data="basketball_double", icon_custom_emoji_id=EMOJI["basketball"]))
...
def football_mode_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Гол (x1.85)", callback_data="football_mode_goal", icon_custom_emoji_id=EMOJI["football"]))
    builder.row(InlineKeyboardButton(text="Промах (x1.5)", callback_data="football_mode_miss", icon_custom_emoji_id=EMOJI["football"]))
    builder.row(InlineKeyboardButton(text="Пенальти (x2)", callback_data="football_mode_penalty", icon_custom_emoji_id=EMOJI["football"]))
...
def bowling_bet_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="0.1", callback_data="bowling_bet_0.1", icon_custom_emoji_id=EMOJI["money"]),
        InlineKeyboardButton(text="0.5", callback_data="bowling_bet_0.5", icon_custom_emoji_id=EMOJI["money"]),
        InlineKeyboardButton(text="1", callback_data="bowling_bet_1", icon_custom_emoji_id=EMOJI["money"])
    )
    builder.row(
        InlineKeyboardButton(text="5", callback_data="bowling_bet_5", icon_custom_emoji_id=EMOJI["money"]),
        InlineKeyboardButton(text="10", callback_data="bowling_bet_10", icon_custom_emoji_id=EMOJI["money"]),
        InlineKeyboardButton(text="50", callback_data="bowling_bet_50", icon_custom_emoji_id=EMOJI["money"])
    )
    builder.row(InlineKeyboardButton(text="Своя сумма", callback_data="bowling_bet_custom", icon_custom_emoji_id=EMOJI["edit"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()

def bowling_choice_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Страйк (x10)", callback_data="bowling_strike", icon_custom_emoji_id=EMOJI["bowling"]))
    builder.row(InlineKeyboardButton(text="Спэр (x5)", callback_data="bowling_spare", icon_custom_emoji_id=EMOJI["bowling"]))
    builder.row(InlineKeyboardButton(text="7+ кеглей (x2)", callback_data="bowling_seven", icon_custom_emoji_id=EMOJI["bowling"]))
    builder.row(InlineKeyboardButton(text="Изменить ставку", callback_data="bowling_change_bet", icon_custom_emoji_id=EMOJI["edit"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()

def guess_number_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="1-10 (x1.5)", callback_data="guess_range_10", icon_custom_emoji_id=EMOJI["guess"]))
    builder.row(InlineKeyboardButton(text="1-50 (x3)", callback_data="guess_range_50", icon_custom_emoji_id=EMOJI["guess"]))
    builder.row(InlineKeyboardButton(text="1-100 (x5)", callback_data="guess_range_100", icon_custom_emoji_id=EMOJI["guess"]))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_games", icon_custom_emoji_id=EMOJI["back"]))
    return builder.as_markup()
...
def play_again_keyboard(game: str):
    ...
    if game == "dice":
        ...
        cb = "blackjack_change_bet"
    elif game == "bowling":
        cb = "bowling_change_bet"
    elif game == "guess_number":
        cb = "game_guess_number"
    else:
        cb = "back_to_games"
    builder.row(InlineKeyboardButton(text="Играть ещё", callback_data=cb, icon_custom_emoji_id=EMOJI[game]))
...
async def send_with_media(message_or_callback, section: str, text: str, reply_markup=None, parse_mode=ParseMode.HTML):
    ...
    if not isinstance(message_or_callback, Message):
        await message_or_callback.message.edit_text(text, parse_mode=parse_mode, reply_markup=reply_markup)
    else:
        await message_or_callback.answer(text, parse_mode=parse_mode, reply_markup=reply_markup)
    return False
...
{e(EMOJI['guess'])} <b>Угадай число</b>
...
{e(EMOJI['blackjack'])} <b>Блэкджек:</b>
  {e(EMOJI['check'])} Побед: {stats['blackjack']['wins']}
  {e(EMOJI['cross'])} Поражений: {stats['blackjack']['losses']}

{e(EMOJI['bowling'])} <b>Боулинг:</b>
  {e(EMOJI['check'])} Побед: {stats['bowling']['wins']}
  {e(EMOJI['cross'])} Поражений: {stats['bowling']['losses']}

{e(EMOJI['guess'])} <b>Угадай число:</b>
  {e(EMOJI['check'])} Побед: {stats['guess_number']['wins']}
  {e(EMOJI['cross'])} Поражений: {stats['guess_number']['losses']}
...
{e(EMOJI['rub'])} <b>Рубли</b> — написать в поддержку
...
@router.callback_query(F.data == "deposit_rub_support")
async def deposit_rub_support(callback: CallbackQuery):
    text = f"""
{e(EMOJI['rub'])} <b>ПОПОЛНЕНИЕ РУБЛЯМИ</b>

Для пополнения баланса рублями, напишите в поддержку:
...
async def withdraw_start(message: Message, state: FSMContext):
    ...
    text = f"{e(EMOJI['withdraw'])} <b>ВЫВОД СРЕДСТВ</b>\n\n{e(EMOJI['wallet'])} Ваш баланс: <b>{user['balance']:.2f} USDT</b>\n\nВведите сумму вывода (мин. 0.5):"
    await send_with_media(message, "withdraw", text, cancel_keyboard())
...
async def blackjack_set_bet(callback: CallbackQuery):
    ...
    user_bets[user_id] = {"game": "blackjack", "bet": bet, "deck": deck, "player_hand": player_hand, "dealer_hand": dealer_hand, "awaiting_custom": False}
...
async def bowling_play(callback: CallbackQuery):
    ...
    if is_win:
        win_amount = bet * mult
        update_balance(user_id, win_amount - bet)
        add_game_stat(user_id, "bowling", True)
        text = f"{e(EMOJI['bowling'])} <b>БОУЛИНГ</b>\n\nВаш выбор: <b>{choice_text}</b>\nРезультат: <b>{result_desc}</b>\n\n{e(EMOJI['check'])} <b>ПОБЕДА!</b>\n\n{e(EMOJI['money'])} +{win_amount - bet:.2f} USDT"
    else:
        update_balance(user_id, -bet)
        add_game_stat(user_id, "bowling", False)
        text = f"{e(EMOJI['bowling'])} <b>БОУЛИНГ</b>\n\nВаш выбор: <b>{choice_text}</b>\nРезультат: <b>{result_desc}</b>\n\n{e(EMOJI['cross'])} <b>ПОРАЖЕНИЕ</b>\n\n{e(EMOJI['money'])} -{bet:.2f} USDT"
...
async def game_guess_number(callback: CallbackQuery):
    user = get_user(callback.from_user.id)
    text = f"""
{e(EMOJI['guess'])} <b>УГАДАЙ ЧИСЛО</b>
...
Множители зависят от диапазона:

Выберите диапазон:
"""
    await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=guess_number_keyboard())
    await callback.answer()

@router.callback_query(F.data.startswith("guess_range_"))
async def guess_range_selected(callback: CallbackQuery, state: FSMContext):
    range_num = int(callback.data.split("_")[-1])
    user_id = callback.from_user.id
    user = get_user(user_id)
    
    mults = {10: 1.5, 50: 3, 100: 5}
    mult = mults[range_num]
    
    secret = random.randint(1, range_num)
    
    await state.update_data(guess_range=range_num, guess_mult=mult, guess_secret=secret)
    
    text = f"""
{e(EMOJI['guess'])} <b>УГАДАЙ ЧИСЛО</b>

Диапазон: <b>1-{range_num}</b>
Множитель: <b>x{mult}</b>

{e(EMOJI['wallet'])} Баланс: <b>{user['balance']:.2f} USDT</b>

Введите вашу ставку и число в формате: <b>ставка число</b>
Например: <b>1 42</b>
"""
    await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=cancel_keyboard())
    await state.set_state(GameStates.waiting_for_guess_number)
    await callback.answer()

@router.message(GameStates.waiting_for_guess_number)
async def guess_number_play(message: Message, state: FSMContext):
    user_id = message.from_user.id
    user = get_user(user_id)
    data = await state.get_data()
    
    range_num = data.get("guess_range")
    mult = data.get("guess_mult")
    secret = data.get("guess_secret")
    
    if not all([range_num, mult, secret]):
        await message.answer(f"{e(EMOJI['cross'])} Сессия игры истекла. Начните заново.", parse_mode=ParseMode.HTML, reply_markup=games_menu_keyboard())
        await state.clear()
        return
    ...
    try:
        ...
        if not match:
            await message.answer(
                f"{e(EMOJI['cross'])} Неверный формат. Используйте: <b>ставка число</b>\nПример: <b>1 42</b>",
                parse_mode=ParseMode.HTML
            )
            return
        ...
        if bet < 0.1:
            await message.answer(f"{e(EMOJI['cross'])} Минимальная ставка 0.1 USDT", parse_mode=ParseMode.HTML)
            return
        ...
        if guess < 1 or guess > range_num:
            await message.answer(f"{e(EMOJI['cross'])} Число должно быть от 1 до {range_num}", parse_mode=ParseMode.HTML)
            return
        ...
        if is_win:
            win_amount = bet * mult
            update_balance(user_id, win_amount - bet)
            add_game_stat(user_id, "guess_number", True)
            text = f"""
{e(EMOJI['guess'])} <b>УГАДАЙ ЧИСЛО</b>

Ваше число: <b>{guess}</b>
...
"""
            ...
            update_balance(user_id, -bet)
            add_game_stat(user_id, "guess_number", False)
...
Ваше число: <b>{guess}</b>
Загаданное число: <b>{secret}</b>
Подсказка: загаданное число <b>{hint}</b>
...
"""
        ...
        new_balance = get_user(user_id)["balance"]
        text += f"\n{e(EMOJI['wallet'])} Баланс: <b>{new_balance:.2f} USDT</b>"
        await message.answer(text, parse_mode=ParseMode.HTML, reply_markup=play_again_keyboard("guess_number"))
        await state.clear()
...
async def admin_stats(callback: CallbackQuery):
    ...
    total_bets = sum(u["stats"][g]["wins"] + u["stats"][g]["losses"] for u in db.values() for g in ["dice", "basketball", "football", "blackjack", "bowling", "guess_number"])
    total_wins = sum(u["stats"][g]["wins"] for u in db.values() for g in ["dice", "basketball", "football", "blackjack", "bowling", "guess_number"])
...
async def admin_media_menu(callback: CallbackQuery):
    ...
    for key, name in sections.items():
        media = media_db.get(key, {})
        status = f"{e(EMOJI['check'])} {media['type']}" if media.get("file_id") else f"{e(EMOJI['cross'])} Нет"
...
async def admin_media_set(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "profile":
        await state.set_state(AdminStates.waiting_for_media_profile)
    elif action == "games":
        await state.set_state(AdminStates.waiting_for_media_games)
    elif action == "deposit":
        await state.set_state(AdminStates.waiting_for_media_deposit)
    elif action == "withdraw":
        await state.set_state(AdminStates.waiting_for_media_withdraw)
    elif action == "support":
        await state.set_state(AdminStates.waiting_for_media_support)
    elif action == "help":
        await state.set_state(AdminStates.waiting_for_media_help)
...
async def admin_media_receive(message: Message, state: FSMContext):
    ...
    if message.photo:
        ...
        media_db[section] = {"type": "photo", "file_id": file_id}
        ...
        await message.answer(
            f"{e(EMOJI['check'])} <b>Фото добавлено в раздел «{section_names.get(section, section)}»!</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=admin_media_keyboard()
        )
    elif message.video:
        file_id = message.video.file_id
        media_db[section] = {"type": "video", "file_id": file_id}
        save_media_db(media_db)
        ...
        await message.answer(
            f"{e(EMOJI['check'])} <b>Видео добавлено в раздел «{section_names.get(section, section)}»!</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=admin_media_keyboard()
        )
    else:
        # Удаляем медиа
        media_db[section] = {"type": None, "file_id": None}
        save_media_db(media_db)
        ...
        await message.answer(
            f"{e(EMOJI['check'])} <b>Медиа удалено из раздела «{section_names.get(section, section)}»!</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=admin_media_keyboard()
        )
