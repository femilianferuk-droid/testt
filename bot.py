...
async def create_lzt_invoice(amount_rub: float, user_id: int):
    async with aiohttp.ClientSession() as session:
        ...
        payment_id = f"vest_{user_id}_{uuid.uuid4().hex[:12]}"
        ...
        try:
            async with session.post(
                ...
                if resp.status == 401:
                    logger.error("LZT API: Unauthorized - проверьте API ключ")
                elif resp.status == 429:
                    logger.error("LZT API: Rate limit exceeded")
                else:
                    logger.error(f"LZT API error: {response_text}")
                return None
...
"""
            ...
            await message.answer(
                ...
                f"Попробуйте позже или используйте Crypto Bot.\n"
                f"Если ошибка повторяется — обратитесь в поддержку @{SUPPORT_USERNAME}",
                parse_mode=ParseMode.HTML
            )
    except ValueError:
        await message.answer(f"{e(EMOJI['cross'])} Введите число", parse_mode=ParseMode.HTML)
...
async def admin_stats(callback: CallbackQuery):
    ...
    total_bets = sum(u["stats"][g]["wins"] + u["stats"][g]["losses"] for u in db.values() for g in ["dice", "basketball", "football", "blackjack"])
    total_wins = sum(u["stats"][g]["wins"] for u in db.values() for g in ["dice", "basketball", "football", "blackjack"])
